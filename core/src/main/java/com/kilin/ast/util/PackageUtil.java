package com.kilin.ast.util;

import com.kilin.ast.CompilationUnit;
...
public class PackageUtil {
    ...
    public static String getClassPath(Node node) {
        for (Node n : node.getChildrens()) {
            if (!Stream.of(TokenType.PACKAGE, TokenType.IMPORT).contains(n.getTokenType())) {
                String name = getClassPath(n);
                if (node.getChildrens().stream().anyMatch(a -> Stream.of(TokenType.PACKAGE, TokenType.IMPORT).contains(a.getTokenType()))) return name;
